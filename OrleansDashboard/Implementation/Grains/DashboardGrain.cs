  ...
  public class DashboardGrain : Grain, IDashboardGrain
  {
    ...
    public async Task<Immutable<string>> GetGrainState(string id, string grainType)
    {
      ...
      try
      {
        ...
        var methodName = grainType.Split(',').Last();

        var getGrainMethod = GrainStateHelper.GenerateGetGrainMethod(GrainFactory, grainId, keyExtension, methodName);
        ...
        foreach (var interfaceType in interfaceTypes)
        {
          try
          {
            var grainMethodParameters = (string.IsNullOrWhiteSpace(keyExtension)) ? new object[] { interfaceType, grainId } : new object[] { interfaceType, grainId, keyExtension };
    ...
    public Task<Immutable<CallableGrainMethod[]>> GetGrainTypes()
    {
      var data = GrainStateHelper.GetCallableGrainMethods().Select(x => new CallableGrainMethod
      {
